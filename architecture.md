...
### `agents/tools/`
Funções ou classes que um agente pode invocar. Cada tool é atômica, reutilizável entre agentes, e responsável por registrar seu próprio passo via `AgentRunService`.
...
class SearchTool:
    ...
    description = "Busca chunks relevantes na base de conhecimento dado uma query."

    def __init__(self, retrieval_service: RetrievalService, run_service: AgentRunService):
        self._retrieval = retrieval_service
        self._runs = run_service

    def run(self, query: str, top_k: int = 5, run_id: str | None = None) -> list[dict]:
        ...
        if run_id:
            self._runs.record_step(
                run_id, tool=self.name, input={"query": query, "top_k": top_k}, output=output
            )
        return output
```
...
O agente recebe o `AgentRunService` diretamente — não via tool — porque é ele quem abre e fecha a execução. As tools recebem o `run_id` para registrar seus passos.
...
class ResearchAgent:
    def __init__(self, search_tool: SearchTool, run_service: AgentRunService, llm_client):
    ...
    def run(self, question: str) -> str:
        ...
        try:
            context = self._search.run(query=question, top_k=5, run_id=run.id)
            context_text = "\n\n".join(c["content"] for c in context)

            answer = self._llm.chat(
                system=SYSTEM_PROMPT,
                user=build_user_prompt(question) + f"\n\nContexto:\n{context_text}",
            )
